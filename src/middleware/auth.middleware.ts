...
export class AuthMiddleware implements IMiddleware<Context, NextFunction> {
  resolve() {
    return async (ctx: Context, next: NextFunction) => {
      ...
      const passwordService = await ctx.requestContext.getAsync<PasswordService>(PasswordService);
      const operateRedisService = await ctx.requestContext.getAsync<OperateRedisService>(OperateRedisService);
      let user;
      try {
